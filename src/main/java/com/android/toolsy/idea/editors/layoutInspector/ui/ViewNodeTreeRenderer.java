...
public class ViewNodeTreeRenderer extends ColoredTreeCellRenderer {
    ...
    public static Font getFontAbleToDisplay(@NotNull String s, @NotNull Font defaultFont) {
        if (!SystemInfo.isMac && !isExtendedAscii(s)) {
            Set<Font> fonts = Sets.newHashSetWithExpectedSize(10);
            FontPreferences fontPreferences = EditorColorsManager.getInstance().getGlobalScheme().getFontPreferences();

            for (int i = 0; i < s.length(); ++i) {
                if (s.charAt(i) > 255) {
                    fonts.add(ComplementaryFontsRegistry.getFontAbleToDisplay(s.charAt(i), 0, fontPreferences, (FontRenderContext) null).getFont());
                }
            }
    ...
    public void customizeCellRenderer(@NotNull JTree tree, Object nodeValue, boolean selected,
        ...
        if (node.getDisplayInfo().getContentDesc() != null) {
            ...
            Font f = getFontAbleToDisplay(node.getDisplayInfo().getContentDesc(), currentFont);
            if (f != currentFont) {
                setFont(f);
            }
