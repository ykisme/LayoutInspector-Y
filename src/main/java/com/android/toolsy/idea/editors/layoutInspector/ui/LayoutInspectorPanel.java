 ...
 */
package com.android.toolsy.idea.editors.layoutInspector.ui;

import com.android.toolsy.idea.editors.layoutInspector.actions.ReInspectorAction;
import com.android.toolsy.idea.editors.layoutInspector.actions.ReInspectorSupport;
import com.google.common.annotations.VisibleForTesting;
import com.android.toolsy.idea.editors.layoutInspector.LayoutInspectorContext;
import com.android.toolsy.idea.editors.layoutInspector.actions.LoadOverlayAction;
import com.android.toolsy.idea.editors.layoutInspector.actions.SetOverlayAlphaAction;
import com.android.toolsy.idea.flagslegacy.StudioFlags;
...
public class LayoutInspectorPanel extends JPanel implements DataProvider, ImageComponentDecorator {
    ...
    private final LayoutInspectorContext mLayoutContext;

    public LayoutInspectorPanel(@NotNull LayoutInspectorContext context) {
        super(new BorderLayout());
        setOpaque(true);
        mLayoutContext = context;
    ...
    private DefaultActionGroup getInspectorActionGroup() {
        ...
        actionGroup.add(new SetOverlayAlphaAction(myPreview));
        actionGroup.add(new ReInspectorAction(new ReInspectorSupport() {
            @Override
            public boolean isCurrentEnable(@Nullable Project project) {
                return mLayoutContext.canReinspector();
            }

            @Override
            public void runReInspector(@Nullable Project project) {
                if (project == null) return;
                // check current client
                if (!ClientUtils.haveClient(AndroidSdkUtils.getDebugBridge(project))) {
                    LayoutInspectorContext.createNotification("no debug devices", NotificationType.ERROR);
                    return;
                }
                if (!mLayoutContext.canReinspector()) {
                    LayoutInspectorContext.createNotification("can not inspector", NotificationType.ERROR);
                    return;
                }
                mLayoutContext.reinspector(project);
            }
        }, mLayoutContext.getParentDispose()));
        return actionGroup;
    }
